...
This is an ASP.NET Core (`net10.0`) + Angular 19 SPA generated from the standard MS template, plus a SQL Server database project. Four C# projects + one Angular project sit side-by-side in the solution:
...
- `PoolLeaderboardEngine/` — pure-domain class library (Elo, Killer game state machine, leaderboard/match repositories, `IDbConnectionFactory`). No ASP.NET dependencies. The server depends on this; the SqlClient connection factory lives in the server project.
...
- `PoolLeaderboard.Database/` — Microsoft.Build.Sql (`.sqlproj`) project. `dotnet build` produces a dacpac; `deploy.sh` publishes it via `sqlpackage`. Schema lives in `*.sql` files (`rating`, `match`).
...
# Database: build dacpac + publish to the dev SQL Server container
./PoolLeaderboard.Database/deploy.sh                # uses $ConnectionStrings__DefaultConnection
```
...
Devcontainer (`.devcontainer/docker-compose.yml`) runs two containers: the dev container (.NET 10 SDK, Node 20, Angular CLI, `sqlcmd`) and a `db` container (mssql 2025) reachable as `db:1433` from inside, `localhost:51433` from the host. The `postCreateCommand` does `dotnet restore`, builds, trusts the dev cert, installs puppeteer's chrome, and runs `deploy.sh` to publish the schema. Forwarded ports: 60125 (Angular), 5166 (HTTP backend), 7114 (HTTPS backend).

To query the dev DB: `sqlcmd -S db -d leaderboard -U sa -P 'YourStrong!Passw0rd' -C` (then type `go` to execute, `quit` to exit).
